...
export class MobileNumberComponent implements OnInit, OnDestroy, ViewWillEnter {
  ...
  private async canShowBiometricLogin(): Promise<boolean> {
    try {
      ...
      const biometricEnabled = this.localStorageService.getItem(storageKeyNameConstants.BIOMETRIC_LOGIN_ENABLED, true);
      const fallbackMobile = this.localStorageService.getItem(storageKeyNameConstants.BIOMETRIC_MOBILE_NUMBER, true);
      return biometricEnabled === true && !!fallbackMobile;
    } catch {
      return false;
    }
  }
  ...
  async loginWithBiometric() {
    ...
    try {
      ...
      const storedMobile =
        (await this.biometricAuthService.getMobileNumberFromCredential()) ||
        this.localStorageService.getItem(storageKeyNameConstants.BIOMETRIC_MOBILE_NUMBER, true);
  ...
  navigateMenu(path: string, event?: Event) {
    const focused = (event?.currentTarget as HTMLElement) || (document.activeElement as HTMLElement | null);
    focused?.blur?.();
