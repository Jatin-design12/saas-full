...
export class AuthService {
  ...
  isAuthenticated() {
    ...
    const otpMatched = this.localStorageService.getItem(storageKeyNameConstants.OTP_MATCH, true);

    if (!userDetails || otpMatched !== true) {
      return false;
    }

    const expiresAt = this.localStorageService.getItem(storageKeyNameConstants.SESSION_EXPIRES_AT, true);
  ...
  markSessionActive(days: number = environment.SESSION_DURATION_DAYS) {
    ...
    this.localStorageService.setItem(storageKeyNameConstants.SESSION_EXPIRES_AT, expiresAt);
  }
}
